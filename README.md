...
- `src/types/`: TypeScript interfaces and type definitions for domain objects (events, preferences, DND windows).
- `src/config/`: Configuration file, primarily for DynamoDB client setup.
- `src/controllers/`: Handles incoming HTTP requests, calls appropriate service functions, and sends responses.
- `src/middleware/`: Contains middleware functions for request validation.
- `src/models/`: Interacts directly with the DynamoDB database, abstracting data access logic.
- `src/routes/`: Defines the API endpoints and maps them to controller functions.
- `src/services/`: Contains the core business logic, orchestrating calls to models and implementing complex decision-making (DND and preference evaluation, merging updates).
- `tests/`: Contains integration tests for the API endpoints.
- `src/app.ts`: The main application entry point, setting up the Express server and routing.
- `dist/`: Compiled JavaScript output (gitignored, generated by `npm run build`).
...
5.  **Create DynamoDB Local Data Directory:**
...
6.  **Configure AWS CLI (for local table creation):**
...
TypeScript: Statically typed superset of JavaScript, compiled to ES2022.
...
Jest + ts-jest: Testing framework with TypeScript support for running integration tests.
